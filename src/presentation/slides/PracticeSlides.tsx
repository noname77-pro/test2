      ...
      <div className="grid h-full grid-cols-[1fr_820px] items-center gap-16">
        <div>
          <Reveal show variant="blur" delay={0.3}>
            <p className="font-display text-[70px] leading-[1.04] font-bold tracking-[-0.03em]">
              Endi nazariy xulosalarni <span className="text-glow">simulyatsiyada</span> tekshiramiz.
            </p>
          </Reveal>
          ...
          <Reveal show variant="up" delay={0.7} className="mt-12">
            <button
              type="button"
              data-interactive
              className="btn btn-primary px-12 py-7 font-display text-[34px] font-bold tracking-wide"
              onClick={() => window.open(SIMULATOR_URL, '_blank', 'noopener')}
            >
              INTERAKTIV TAJRIBANI BOSHLASH
...
export function ExampleSlide({ step }: SlideProps) {
  return (
    <SlideShell number={13} eyebrow="Masala yechamiz" title="Namunaviy masala">
      <div className="grid h-full grid-cols-[560px_1fr] gap-12">
        <div className="flex flex-col gap-6">
          <Reveal show variant="left" delay={0.25} className="card px-9 py-7">
            ...
            <div className="mt-5 border-t border-white/10 pt-4">
              <p className="font-display text-[32px] font-semibold whitespace-nowrap">Jismning tezlanishini toping.</p>
              <Tex math="a = \,?" className="mt-1 block text-[42px] text-glow" />
            </div>
          </Reveal>
        ...
        <div className="flex flex-col justify-center gap-4">
          {SOLUTION.map((line, i) => (
            <Reveal key={line} show={step >= i + 1} variant="left" className={`card flex items-center gap-7 px-8 ${i === 4 ? 'py-5' : 'py-6'}`}>
              ...
              <Tex math={line} className="text-[48px]" />
            </Reveal>
          ))}
        </div>
      </div>
...
const QUIZ = [
  ...
  { q: 'Normal reaksiya kuchi nimaga teng?', a: `${T.N} = mg\\cos\\alpha` },
  { q: 'Ishqalanish kuchi formulasi qanday?', a: `${T.Fishq} = \\mu ${T.N}` },
  { q: 'Jismning sirpanish sharti qanday?', a: '\\tan\\alpha > \\mu' },
  { q: 'α burchak ortsa F_x qanday o‘zgaradi?', a: '\\text{Ortadi}' },
]
